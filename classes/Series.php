...
class Series{
	...
	public function createSeries($name, $volume, $collection_id, $comicvine_info){
		...
		$this->series_id = $this->db->addSeries($this->name, $this->volume, $this->collection_id, $this->year, $this->comicvine_series_id, $this->comicvine_series_full);
	...
	public function getDisplayText(){
		return "{$this->name} vol.{$this->volume} ({$this->year})";
	}
	...
	public function get($series_id){
		...
		if($series){
			...
			if($series['name']){ $this->name = $series['name']; }
	...
	protected $name = false;//my series name, not comicvine's name
